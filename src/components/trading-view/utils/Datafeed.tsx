...
const makeDataFeed = (tokenId, tokenSymbol, poolId, pools = false) => {
  ...
  return {
    ...
    async subscribeBars(
      ...
      // If re-subscribing, clean up previous listeners/rooms
      if (subscriptions[subscriberUID]) {
        const prev = subscriptions[subscriberUID];
        // remove listeners
        if (prev.listeners) {
          prev.listeners.forEach(({ room, handler }) => {
            try { SocketService.off(room, handler); } catch {}
          });
        }
        // leave rooms
        if (prev.rooms) {
          prev.rooms.forEach((room) => {
            try { SocketService.leaveRoom(room); } catch {}
          });
        }
        prev.stop();
      ...
      const subRecord = {
        stop: () => { stopped = true; },
        listeners: [] as Array<{ room: string; handler: (d: any) => void }>,
        rooms: [] as string[],
      };
      subscriptions[subscriberUID] = subRecord;

      // Inform chart to reset cache when resolution changes
      try { onResetCacheNeededCallback?.(); } catch {}
      ...
      if (socket) {
        if (!pools || !pools?.length) {
          const priceRoom = `price-by-token:${tokenId}`;
          const txnRoom = `transaction:${tokenId}:${poolId}`;
          SocketService.joinRoom(priceRoom);
          SocketService.joinRoom(txnRoom);
          subRecord.rooms.push(priceRoom, txnRoom);

          const txnHandler = (txns) => handleVolume(txns);
          const priceHandler = (data: any) => { if (data.pool) realtimeHandler(data); };
          SocketService.on(txnRoom, txnHandler);
          SocketService.on(priceRoom, priceHandler);
          subRecord.listeners.push({ room: txnRoom, handler: txnHandler }, { room: priceRoom, handler: priceHandler });
        } else {
          const txnRoomAll = `transaction:${tokenId}`;
          SocketService.joinRoom(txnRoomAll);
          subRecord.rooms.push(txnRoomAll);
          const txnAllHandler = (txns) => { console.log(txns); handleVolume(txns); };
          SocketService.on(txnRoomAll, txnAllHandler);
          subRecord.listeners.push({ room: txnRoomAll, handler: txnAllHandler });

          pools.forEach((pool) => {
            const priceRoom = `price:${pool}`;
            SocketService.joinRoom(priceRoom);
            subRecord.rooms.push(priceRoom);
            const priceHandler = (data: any) => { if (data.pool) realtimeHandler(data); };
            SocketService.on(priceRoom, priceHandler);
            subRecord.listeners.push({ room: priceRoom, handler: priceHandler });
          });
        }
      }
    },
